...
public class EAUBoardBuilder implements BoardBuilder {
	...
	public Board buildBoard() {
		...
		pieces.stream().filter(p -> (p.getX() == randomVal.get(0)) || (p.getX() == randomVal.get(1))).forEach(p -> p.setColor(Color.BLACK));
		...
		pieces.stream().filter(p -> (p.getX() == randomVal.get(0)) || (p.getX() == randomVal.get(1))).forEach(p -> p.setColor(Color.WHITE));
